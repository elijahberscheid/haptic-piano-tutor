...
//============================================================================
// Credit to the ECE362 staff for providing this library. We only modified it slightly.
...
#define LCD_CS_SET  do { while((SPI->SR & SPI_SR_BSY) != 0); GPIO_TYPE_CS->BSRR=1<<LCD_CS; } while(0)
#define LCD_RS_SET  GPIO_TYPE->BSRR=1<<LCD_RS
#define LCD_RESET_SET GPIO_TYPE->BSRR=1<<LCD_RESET

#define LCD_CS_CLR  GPIO_TYPE_CS->BRR=1<<LCD_CS
...
{
    // Assert reset
    GPIO_TYPE->BRR = 1<<LCD_RESET;
    nano_wait(100000000);
    GPIO_TYPE->BSRR = 1<<LCD_RESET;
    nano_wait(50000000);
}
